...
## TITAN Reclamation Beta Synthesis Contract (2026-05-17)
- [XML CONTRACT]: `shared/titan-reclamation-beta-synthesis.xml` adds a machine-readable mission contract for the next Claude Code pass. It binds the Organic Vegas overhaul to deterministic execution, no fabricated dependency/path/script assumptions, inert-reference protection for imagery, and explicit uncertainty/error-escalation protocols.
- [PHASE GATES]: The contract stages the work through deterministic audit/path validation, core infrastructure reclamation, synthetic asset and musical evolution, mathematical validation/stress, and mobile finalization/hardening.
- [PATH REALITY]: `apps/frontend/src/erk/receptor.ts`, `apps/frontend/src/erk/leitmotifs.ts`, `apps/frontend/src/components/game/OrganicVegasScene.tsx`, `apps/backend/src/gameRoom.ts`, `apps/frontend/src/audio/spectralGenome.ts`, `packages/farkle-engine/src/monteCarlo.ts`, and `packages/farkle-engine/src/rtpConfig.ts` are the current verified anchors. `scripts/simulateEconomy.ts` is not assumed to exist; any economy script must be a proven wrapper around existing Monte Carlo/RTP APIs or trigger the Uncertainty Protocol.
- [CLAUDE HANDOFF]: `shared/cc-prompt.md` now requires Claude Code to read the XML contract before implementation and to treat it as the deterministic execution layer beneath the source-of-truth hierarchy.

## TITAN Phase 2 — Infrastructure Reclamation (2026-05-17)
- [DIRECTOR DECISIONS]: Q1/d — dream/ is authoritative; core/ is reference only. Q2/c — Real 2-player WebSocket multiplayer, live Railway backend. Q3/b — All post_bank-hook genre modules (casino_bank, roguelike_facets, rpg_classes, economy, narrative, liveops, vfx_juice, adaptive_ai, progression, telemetry) are payout-neutral wrappers until 10,000-session Monte Carlo gate passes. Q4/a — economy, analytics, ads, compliance packages migrated into dream/packages/.
- [THEME REGISTRY]: `packages/dream-core/src/ThemeRegistry.ts` created. Maps design_tokens.json r1 → CSS custom properties, Three Tissues palette, shader uniform constants (tileFresnel, organicMembrane, casinoGlint, syncField, voidNoise), and AV event bindings. `injectTheme()` called in `apps/frontend/src/main.tsx` at boot.
- [SIMULATE ECONOMY]: `scripts/simulateEconomy.ts` created. Thin wrapper over Sacred Core monteCarlo.ts + rtpConfig.ts (already existed in packages/farkle-engine/src/). Runs 10,000 sessions per GameMode. Exit code 1 if any mode exceeds 2% RTP deviance gate.
- [LIVEOPS PACKAGES]: `packages/economy/`, `packages/analytics/`, `packages/ads/`, `packages/compliance/` added to dream/ with @adabt/ package names. Migrated from core/ with no logic changes; all packages are reference-only until wired to backend endpoints.
- [BUILD]: Frontend TypeScript check and `npm run build` pass clean (0 TS errors, 144 modules, 8.18s).
- [BLOCKED]: Phase 3 (PhraseGenerator.ts, 2-player multiplayer wiring), Phase 4 (MC simulation run + RTP report), Phase 5 (frame-pacing audit, beta release) await Director trigger.

## TITAN Phase 3 — Synthetic Asset & Musical Evolution (2026-05-17)
- [PHRASE GENERATOR]: `apps/frontend/src/erk/phraseGenerator.ts` created. Deterministic 4/8-bar phrase gen from EmotionalStateVector + leitmotif registry. Cluster selection: resolution→narrative, chaos→profane, (hi-tension+momentum)→baroque, (lo-tension+lo-chaos)→sacred, default→atmospheric. Tempo mutations bounded by MutationEnvelope.maxTempoDrift × momentum. Key transpositions bounded by maxTransposition, biased by resolution. xorshift32 PRNG (separate from Sacred Core seededRng). `deriveSeed(serverTick, seedCursor)` produces per-turn seed without Math.random().
- [MULTIPLAYER WIRING]: `apps/frontend/src/hooks/useOrganicMultiplayer.ts` protocol corrected to match gameRoom.ts broadcast types. TURN_RESULT (phantom) replaced with TURN_CHANGE + CHAIN_RESULT. SESSION_END (provably-fair seed reveal + payout), BOARD_UPDATE (grid sync), ENERGY_ZERO, GAME_STARTED (gameMode + roles) all handled. State extended: grid, gameMode, lastChainResult, sessionResult, isWinner.
- [BUILD]: Frontend TypeScript and production build clean (0 errors, 144 modules, 7.96s). gameRoom.ts untouched (Sacred Core compliance).
- [NEXT — Phase 4]: Run simulateEconomy.ts for MC report; Phase 5: frame-pacing audit + beta hardening.

## TITAN Phase 4 — Determinism Audit + MC Validation (2026-05-17)
- [MC REPORT]: `scripts/simulateEconomy.ts` fixed — previous normalizer was tautological (deviance always 0%). Now uses two-phase calibration vs. production comparison. Result: all 8 modes pass 2% gate (deviance 1.30% — expected sampling variance; all genres still rtpGated=true in beta). `packages/farkle-engine/src/monteCarlo.ts` UNTOUCHED (Sacred Core).
- [TEST RUNNER]: `packages/farkle-engine/package.json` now has `"test": "npx tsx --test src/farkleScorer.test.ts"`. All 16 regression cases pass: single die, pairs, triplets, four-of-kind, five-of-kind, six-of-kind, straight, three-pairs, W6 Two-Triplets.
- [DETERMINISM FIX]: `Math.random()` removed from all gameplay-affecting paths. `ItemSystem.ts` now requires a seeded `rng: () => number` injected via constructor. `GameSessionManager.ts` seeds it with xorshift32. `RhythmEngine.ts` (AGROS) uses crypto-entropy-seeded xorshift32 internally. `gameAudio.ts` noise buffers (audio-only, non-deterministic acceptable) unchanged.
- [GAME LOOP GAP — ESCALATE]: Frontend `OrganicVegas.tsx` calls `scoreFarkle(allSixDice)` immediately on each roll — this treats every roll as a 6-die combo, producing ~90% farkle rate. Backend `gameRoom.ts` implements grid-chain-matching (SUBMIT_CHAIN). These are different games. Frontend must implement chain-selection UI or be redesigned as solo dice-roller. Requires Director decision before Phase 5 implementation.
- [BUILD]: npm run build clean (144 modules, 0 TS errors). Backend tests 3/3 pass. Farkle scorer 16/16 pass.
